...
4. Phase 3 - Data Processing and Transformation  
   ...
   - Success criteria: all columns renamed correctly, ages numeric, encounters flagged, transformation rules applied; all unit tests pass

5. Phase 4 - Age Categorization and Data Aggregation  
   - Implement detailed age categorization (Newborn through Geriatric)  
   - Implement simple age categorization (Pediatric vs Adult)  
   - Implement aggregation for all 35 competency categories grouped by student  
   - Implement success tracking for Intravenous (IV) and Orotracheal Intubation  
   - Implement patient type classification using defined impression and complaint lists  
   - Implement rotation site counting  
   - Implement age by site breakdown  
   - Implement Cohen/CCMC pediatric patient report  
   - Write unit tests for age categorization, each aggregation function, and success tracking  
   - Success criteria: all 35 categories aggregate correctly, success percentages calculate accurately, patient type classifications match defined lists; all unit tests pass

6. Phase 5 - Completion Status and Report Generation  
   - Implement completion status check against minimum requirements for all 35 categories  
   - Implement percent success calculation for IV and Intubation  
   - Implement full report data assembly combining all aggregated sections  
   - Write unit tests for completion status logic and report assembly  
   - Success criteria: completion status correctly marks Completed or Not Completed for every category, success percentages are accurate; all unit tests pass

7. Phase 6 - Authentication and UI Implementation  
   - Implement instructor authentication screen with Universal ID and Employee Number fields  
   - Implement credential validation against the hardcoded authorized list  
   - Implement student selection interface displaying all students from the retrieved data  
   - Implement optional date range filter inputs with mm-dd-yyyy format validation  
   - Implement processing progress display with status messages  
   - Implement report display view showing all competency categories with counts, minimums, successful attempts, percent success, and completion status  
   - Implement rotation site breakdown display  
   - Implement age by site breakdown display  
   - Implement Cohen/CCMC pediatric count display  
   - Apply Viridis color scheme throughout all UI components  
   - Write unit tests for authentication logic and date validation  
   - Success criteria: authentication works correctly, student selection populates from live data, date filtering works, report displays all required data sections with correct color scheme; all unit tests pass

8. Phase 7 - Print and PDF Output  
   - Implement browser print styling with clean, professional layout suitable for accreditation documentation  
   - Implement PDF download using a PDF generation library  
   - Ensure report layout renders correctly in both print and PDF formats  
   - Success criteria: printed output is clean and professional, PDF downloads correctly with all data intact, layout is suitable for accreditation submission

9. Phase 8 - Integration Testing  
   - Carry out extensive integration testing with Playwright or similar framework  
   - Test full workflow: authentication, data retrieval, student selection, date filtering, report generation, print, and PDF download  
   - Test error handling: invalid credentials, API failures, empty data, invalid date formats  
   - Test edge cases: students with zero counts, students with all categories completed, date ranges returning no data  
   - Fix all defects discovered during testing  
   - Success criteria: all integration tests pass, no critical defects remain, full workflow operates end to end without errors

10. Only complete when the MVP is finished and tested, with the server running and ready for the user

## Coding Standards

1. Use latest versions of libraries and idiomatic approaches as of today  
2. Keep it simple - NEVER over-engineer, ALWAYS simplify, NO unnecessary defensive programming. No extra features beyond what is specified - focus on simplicity.  
3. Be concise. Keep README minimal. No emojis ever.  
4. Use TypeScript throughout the project with strict type checking enabled  
5. All data structures must have explicit type definitions - submissions, students, competency categories, aggregation results, report data  
6. Follow NextJS app router conventions and file-based routing  
7. Use functional components and React hooks exclusively - no class components  
8. Keep components small and focused on a single responsibility  
9. All business logic (API calls, data transformation, aggregation, completion checks) must reside in utility modules separate from UI components  
10. All utility functions must have corresponding unit tests  
11. Name files and folders in kebab-case; name components in PascalCase; name functions and variables in camelCase  
12. Keep all configuration values (API key, form ID, minimum counts, credential list, column mappings, patient type lists) in dedicated constants files - never scatter magic values across components or utilities  
13. Handle API errors gracefully with clear user-facing status messages - no silent failures  
14. Ensure the report output is accessible and readable at standard print sizes for accreditation documentation purposes  
15. Use semantic HTML elements where appropriate for accessibility  
16. All colors must reference the defined Viridis color scheme constants - no hardcoded color values in components  
